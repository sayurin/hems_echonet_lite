...
デバイスクラスは、検出されると自動的に処理されます。

- **検証済み** — 実機で動作確認済みです。
- **その他** — 実機で未検証のため、機器によっては正しく動作しない場合があります。

### 検証済みデバイスクラス

| クラスコード | 機器 | HA プラットフォーム |
|------------|------|------------------|
| 0x0130 | 家庭用エアコン | Climate |
| 0x0135 | 空気清浄機 | Fan |
| 0x013A | シーリングファン | Fan、Light |
| 0x0263 | 電動雨戸・シャッター | Cover |
| 0x026B | 電気温水器 | Water Heater |
| 0x026F | 電気錠 | Lock |
| 0x0272 | 瞬間式給湯器 |
| 0x0279 | 住宅用太陽光発電 |
| 0x027B | 床暖房 |
...
### Ceiling Fan — シーリングファン（0x013A）

1 つの ECHONET オブジェクトが Fan エンティティを公開し、ランプ（EPC 0xF3）がある場合は同じデバイス上に Light エンティティも公開します。制御書き込みはすべて `ceiling_fan_set_properties` による 1 回の SetC です（既定は無音）。

**ファン**

- **オン / オフ**: 動作状態（EPC 0x80）。オフ時も直前の風量は保持されます
- **風速**: 10 段階（EPC 0xF0）、10%〜100%
- **風向**: 下向き / 上向き（EPC 0xF1）。Home Assistant の Forward / Reverse に対応
- **プリセットモード**: 通常、自然風（EPC 0xF2）。自然風は風量を変化させます（首振りではありません）

**照明**（EPC 0xF3 が広告されている場合のみ）

- **オン / オフ**: 照明（EPC 0xF3）。ファンの電源とは独立
- **エフェクト**: 主照明 / 常夜灯（EPC 0xF4）
- **明るさ**: 通常モードは 1〜100%（EPC 0xF5）、常夜灯モードは 低 / 中 / 高（EPC 0xF7）
- **色温度**: 暖色（0）〜寒色（100）の連続値（EPC 0xF6）、2700〜6500 K にマッピング
...
- **適応プロパティポーリング**: 60 秒の基準間隔から開始し、応答遅延と失敗回数に応じて機器ごとに間隔を調整します。
- **高頻度レーン**: 瞬時値 EPC は必要に応じて fast poll ループで別スケジュールされます。
- **適応バッチサイズ**: 機器が部分応答を返す場合、学習した上限に基づいて EPC 要求をチャンク化します。
- **実効ポーリング集合**: 無効化された entity はランタイムのポーリング対象から除外されます。
...
- 実機で未検証のデバイスクラスは、正しく動作しない場合があります。
...
1. 一部の機器は応答に時間がかかることがあります。再探索は 1 時間ごとに実行されるため、数分待ってから再確認してください。
2. **設定 → デバイスとサービス → HEMS Echonet Lite → ⋮ → 再読み込み** からインテグレーションを再読み込みしてみてください。
3. 機器が ECHONET Lite に対応しているか確認してください。一部の家電はデフォルトで ECHONET Lite が無効になっており、メーカーのアプリや設定から有効化が必要な場合があります。
